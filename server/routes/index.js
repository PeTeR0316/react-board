...
router.post('/join', (req, res) => {
    const insertId = req.body.inId; //아이디
    const insertPassword = req.body.inPassword; //비밀번호
    const insertName = req.body.inName; //이름

    let joinSQL = `INSERT INTO member (id, pwd, name) value ('${insertId}', '${insertPassword}', '${insertName}')`;

    connection.query(joinSQL, (err, rows, fields) => {
        // res.send(rows);
        ...
        console.log(Date() + " 회원가입 ok!");
    });
});

//로그인
router.post('/login', (req, res) => {
    const loginId = req.body.loginId; //아이디
    const loginPassword = req.body.loginPassword; //비밀번호

    let loginSQL = `SELECT count(*) AS count FROM member WHERE id = '${loginId}' AND pwd= '${loginPassword}'`;

    connection.query(loginSQL, (err, rows, fields) => {
        res.send(rows);
        console.log(Date() + " 로그인 시도 ok!");
    });
})
